---
title: "R Programming Midterm 201701 - 01"
author: "coop711"
date: "`r Sys.Date()`"
...
midterm.roll <- read.xlsx("../data/midterm_01.xlsx", 
                          sheetIndex = 1, 
                          startRow = 3, 
                          endRow  = 32, 
                          colIndex = c(4, 10:49), 
...
midterm.tbl <- sapply(midterm, 
                      table, 
                      useNA = "no")
pander(midterm.tbl[, 1:20])
pander(midterm.tbl[, 21:40])
str(midterm.tbl)
midterm.ptbl <- round(apply(midterm.tbl, 
                            2, 
                            prop.table) * 100, 
                      digits = 0)
pander(midterm.ptbl[, 1:20])
pander(midterm.ptbl[, 21:40])
str(midterm.ptbl)
...
pander(midterm.marks.rw[, 1:20])
pander(midterm.marks.rw[, 21:40])
```
...
pander(data.frame("순번" = 1:nrow(midterm), midterm.score)[1:20])
# pander(data.frame("학번" = midterm.roll$id, midterm.score)[1:20])
pander(data.frame("순번" = 1:nrow(midterm), midterm.score)[21:40])
# pander(data.frame("학번" = midterm.roll$id, midterm.score)[21:40])
...
pander(data.frame("순번" = 1:nrow(midterm), "점수" = midterm.score))
```
